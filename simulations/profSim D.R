run.sim <- function(config, noise_type ="sim", noise_dat = NULL) {
  ...
  for(rep in 1:replications) {
    ...
    for(m in 1:length(clusters)) {
      ...
      try(mle <- optimizeSelected(observed, subCov, threshold[cluster$row],
                                  selected = selected,
                                  projected = NULL,
                                  stepRate = 0.6,
      ...
      for(methodind in 1:2) {
        ...
        nullfit <- NULL
        try(nullfit <- optimizeSelected(observed, subCov, threshold[cluster$row],
                                        projected = 0,
                                        selected = selected,
                                        stepRate = 0.6,
                                        coordinates = cluster[, 1:3],
        ...
        try(profile <- optimizeSelected(observed, subCov, threshold[cluster$row],
                                        selected = selected,
