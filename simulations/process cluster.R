...
fcoverplot <- ggplot(forplot) +
  ...
  geom_segment(aes(y = pmin(cover + coversd * quantile, 1), yend = pmin(cover + coversd * quantile, 1),
                   x = snr - ciwidth, xend = snr + ciwidth, col = method)) +
  ggtitle("fMRI Residuals") + theme(legend.position = "none") +
  scale_colour_brewer(palette = "Set1") + ylim(0.9, 1)
fcoverplot

fpowerplot <- ggplot(forplot) +
...
# pdf("figures/fmriciplots.pdf",pagecentre=T, width=8, height=3 ,paper = "special")
...
fmse <- ggplot(forplot, aes(x = snr, y = mse, col = method)) +
  facet_grid(grp_size ~ pthreshold, labeller = "label_both") +
  ...
  theme_bw() + ggtitle("fMRI Residuals") +
  theme(legend.position = "none") + ylab("RMSE")
fmse

fbias <- ggplot(forplot, aes(x = snr, y = bias, col = method)) +
  facet_grid(grp_size ~ pthreshold, labeller = "label_both") +
  geom_line(aes(linetype = method)) + geom_point() +
  ...
  ggtitle("fMRI Residuals")
fbias

pdf("figures/fmriestplots.pdf",pagecentre=T, width=8, height=3 ,paper = "special")
...
# Joint Figures ------------
pdf("figures/fmricover.pdf",pagecentre=T, width=8, height=2.6 ,paper = "special")
gridExtra::grid.arrange(scoverplot, fcoverplot, nrow = 1)
dev.off()

pdf("figures/fmripower.pdf",pagecentre=T, width=8, height=2.6 ,paper = "special")
gridExtra::grid.arrange(spowerplot, fpowerplot, nrow = 1)
dev.off()

pdf("figures/fmrimse.pdf",pagecentre=T, width=8, height=2.6 ,paper = "special")
gridExtra::grid.arrange(smse, fmse, nrow = 1)
dev.off()

pdf("figures/fmribias.pdf",pagecentre=T, width=8, height=2.6 ,paper = "special")
gridExtra::grid.arrange(sbias, fbias, nrow = 1)
dev.off()
