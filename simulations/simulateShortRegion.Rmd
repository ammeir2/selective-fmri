    ...
    for (shape_i in 1:n_shapes){
      for (sig_i in 1:n_sigs){
        ...
        dat_ind = (grp_i-1)*n_musize*n_shapes*n_sigs+
                  ((theta_type-1)*n_shapes*n_sigs)+ (shape_i-1)*n_shapes+sig_i 
        mu_vec = mu_shape[shape_i,]*mu_size[musize_i]
        sampParams = setSamplingParameters(na=ns[grp_i], nb = ns[grp_i], 
                                           mudiff=mu_vec,
                                           sig = sigCorrel[[sig_i]],
